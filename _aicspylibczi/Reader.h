 ...
 *
 * This C++ extension wraps Zeiss's libCZI providing a minimal interface for
 * working with CZI files. There main class is Reader. Reader is constructed
 * with a FileHolder class which is simply a class that contains a FILE *.
 *
 ...
 *         // img is a vector containing one ImageBC* pointing to an Image<T>
 * which contains the whole mosaic image
 *     }
 * }
 *
 * \endcode
 ...
 *
 * This class opens has the functionality to read mosaic files as well as single
 * and multi-scene files. It relies on libCZI from Zeiss and our intent is that
 * this code will enable the reading of CZI files generated by Zeiss systems. If
 * you have a file that causes problems please contact us and share the file if
 * possible and we will do our best to enable support. Ideally make an issue on
 * the github repo https://github.com/elhuhdron/pylibczi
 */
class Reader
{
...
public:
   ...
   *
   * This test is done by checking the maximum mIndex, mosaic files will have
   * mIndex > 0
   *
   ...
   * B = The B-dimension ("block") - its use is deprecated, but it still gets
   * returned 🙃
   *
   * The internal structure suggests that a given dimension could start at a
   * value other than zero. Although I have yet to observe this we return a pair
   * containing both the start and the size for the index.
   *
   * @return A vector< map< DimensionIndex, pair<int(start), int(end)> > > which
   * conforms to [start, end) meaning for( int i = start; i < end ; i++) is
   * valid. If the shape of the scenes is consistent there will be one element
   * in the vector. If the shape is not consistent each Scene will be present in
   * the vector with it's explicit dimensions.
   */
  Reader::DimsShape readDimsRange();

  /*!
   * @brief Get the Dimensions in the order that they appear.
   * @return a string containing the dimensions. Y & X are included for
   * completeness despite not being DimensionIndexes.
   */
  std::string dimsString();

  /*!
   * @brief Get the size of the dimensions in the same order as dimsString()
   * @return a vector containing the dimension sizes. Y & X are included for
   * completeness. If the dims are not consistent a vector of -1 values is
   * returned. The used should then use readDimsRange()
   */
   ...
   * @return a tuple, the first index is if S is defined the second is the start
   * index and the third is the number of S indexs
   */
   ...
   * @return A map containing the DimIndex with a pair of integers start, end
   * such that for(int i = start; i < end ; i++) is valid.
   */
  Reader::DimIndexRangeMap sceneShape(int scene_index_);

  /*!
   * @brief Get the metadata from the CZI file.
   * @return A string containing the xml metadata from the file. Y & X are
   * included for completeness despite not being DimensionIndexes.
   */
  std::string readMeta();

  /*!
   * @brief Given a CDimCoordinate, even an empty one, return the planes that
   * match as a numpy ndarray
   *
   * The plane coordinate acts as a constraint, in the example below the dims
   * are a set of constraints. What this means is that every image taken that
   * has Z=8, T=0, C=1 will be returned.
   * @code
   *    FILE *fp = std::fopen("mosaic_file.czi", "rb");
   *    czi = pylibczi::Reader(fp);
   *    auto dims = CDimCoordinate{ { DimensionIndex::Z,8 } , {
   * DimensionIndex::T,0 }, { DimensionIndex::C,1 } }; auto img_coords_dims =
   * czi.cziread_selected(dims)
   * @endcode
   *
   * @param plane_coord_ A structure containing the Dimension constraints
   * @param index_m_ Is only relevant for mosaic files, if you wish to select
   * one frame.
   */
  std::pair<ImagesContainerBase::ImagesContainerBasePtr,
            std::vector<std::pair<char, size_t>>>
  readSelected(libCZI::CDimCoordinate& plane_coord_, int index_m_ = -1);

  /*!
   * @brief provide the subblock metadata in index order consistent with
   * readSelected.
   * @param plane_coord_ A structure containing the Dimension constraints
   * @param index_m_ Is only relevant for mosaic files, if you wish to select
   * one frame.
   ...
   */
  libCZI::IntRect readSubblockRect(libCZI::CDimCoordinate& plane_coord_,
                                   int index_m_ = -1);

  /*!
   * @brief If the czi file is a mosaic tiled image this function can be used to
   * reconstruct it into an image.
   * @param plane_coord_ A class constraining the data to an individual plane.
   * @param scale_factor_ (optional) The native size for mosaic files can be
   * huge the scale factor allows one to get something back of a more reasonable
   * size. The default is 0.1 meaning 10% of the native image.
   * @param im_box_ (optional) The {x0, y0, width, height} of a sub-region, the
   * default is the whole image.
   * @return an ImagesContainerBasePtr containing the raw memory, a list of
   * images, and a list of corresponding dimensions
   *
   * @code
   *    FILE *fp = std::fopen("mosaicfile.czi", "rb");
   *    czi = pylibczi::Reader(fp);
   *    auto dims = czi.read_dims();  // dims = {{DimensionIndex::T, {0, 10}},
   * {DimensionIndex::C, {0,3}}
   *    // the dims above mean that T and C need to be constrained to single
   * values
   *
   *    auto c_dims = CDimCoordinate{ { DimensionIndex::T,0 }, {
   * DimensionIndex::C,1 } }; auto img_p = czi.read_mosaic( c_dims,
   * scaleFactor=0.15 ); if(img_p->is_type_match<uint16_t>(){ // supported types
   * are uint8_t, uint16_t, and float std::shared_ptr<uint16_t> img =
   * img_p->get_derived<uint16_t>(); // cast the image to the memory size it was
   * acquired as
   *        // do what you like with the Image<uint16_t>
   *    }
   * @endcode
   */
  ImagesContainerBase::ImagesContainerBasePtr readMosaic(
    libCZI::CDimCoordinate plane_coord_,
    float scale_factor_ = 1.0,
    libCZI::IntRect im_box_ = { 0, 0, -1, -1 });
  // changed from {.w=-1, .h=-1} to above to support MSVC and GCC - lagging on
  // C++14 std
   ...
   */
  static char dimToChar(libCZI::DimensionIndex di_)
  {
    return libCZI::Utils::DimensionToChar(di_);
  }
  ...
  /*!
   * Get the full size of the mosaic image without scaling. If you're selecting
   * a sub-region it must be within the box returned.
   * @return an IntRect {x, y, w, h}
   */
...
private:
  ...
  {
    return (info_.logicalRect.w == info_.physicalSize.w &&
            info_.logicalRect.h == info_.physicalSize.h);
  }

  // void getMemory(SubblockIndexVec& matches_);

  static bool isValidRegion(const libCZI::IntRect& in_box_,
                            const libCZI::IntRect& czi_box_);
