...
class ImageFactory
{
  ...
  using CtorMap = std::map<libCZI::PixelType,
                           std::function<std::shared_ptr<Image>(
                             ...
                             int mIndex_)>>;
  using SplitCtorMap =
    std::map<libCZI::PixelType,
             std::function<std::shared_ptr<Image>(std::shared_ptr<Image> img_,
                                                  int channel_)>>;
...
public:
  ImageFactory(libCZI::PixelType pixel_type_, size_t pixels_in_all_images_)
    : m_imgContainer(
        ImagesContainerBase::getTypedAsBase(pixel_type_, pixels_in_all_images_))
  {}
  ...
  template<typename T>
  static std::shared_ptr<TypedImage<T>> getDerived(
    std::shared_ptr<Image> image_ptr_)
  {
    if (!image_ptr_->isTypeMatch<T>())
      throw PixelTypeException(
        image_ptr_->pixelType(),
        "TypedImage PixelType doesn't match requested memory type.");
