...
namespace pylibczi {
  ...
  public:
      ...
      TypedImage(std::vector<size_t> shape_,
          ...
          T* mem_ptr_,
          int m_index_)
          :Image(shape_, pixel_type_, plane_coordantes_, box_, m_index_), m_array(mem_ptr_)
      {
          if (!isTypeMatch<T>())
  ...
  TypedImage<T>::operator[](const std::vector<size_t>& idxs_xy_)
  {
      if (idxs_xy_.size()!=m_shape.size())
          throw ImageAccessUnderspecifiedException(
              idxs_xy_.size(), m_shape.size(), "from TypedImage.operator[].");
      ...
      size_t channels_)
  {
      ...
      if (m_pixelType==libCZI::PixelType::Gray8 ||
          m_pixelType==libCZI::PixelType::Gray16 ||
          m_pixelType==libCZI::PixelType::Gray32Float)
          std::memcpy(m_array, lckScoped.ptrDataRoi, lckScoped.size);
      else {
          SourceRange<T> sourceRange(channels_,
              static_cast<T*>(lckScoped.ptrDataRoi),
              (T*) (sEnd),
              lckScoped.stride,
              size_.w);
          TargetRange<T> targetRange(
              channels_, size_.w, size_.h, m_array, m_array+length());
